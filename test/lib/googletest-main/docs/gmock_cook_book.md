...
class Foo {
  ...
  virtual int Add(Task x);
  virtual int Add(int times, Task x);
...
class MockFoo : public Foo {
  ...
  MOCK_METHOD(int, Add, (Task x), (override));
  MOCK_METHOD(int, Add, (int times, Task x), (override));
...
class MockFoo : public Foo {
  ...
  using Foo::Add;
  MOCK_METHOD(int, Add, (Task x), (override));
  // We don't want to mock int Add(int times, Task x);
  ...
};
```
